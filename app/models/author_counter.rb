class AuthorCounter < ApplicationRecord
  ...
  # AuthorCounter.update_author_counter(Time.parse("2020-06-01"))
  def self.update_author_counter(start_time)
    TData.where.not(con_author:["",nil,"[]"]).where("data_time > ?",start_time).select("data_id,con_author,con_time").find_each do |x|
      authors = JSON.parse(x.con_author) rescue nil
      next if authors.blank?
      date = x.con_time.to_date
      authors.each do |author|
