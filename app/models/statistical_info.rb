...
# TODO: 已经同步完成的历史的数据都要情况，释放redis的内存。
# TODO: 引入新的计数器。"archon_host_task_counter_#{ip}"	sorted set  member=hour  score=count
class StatisticalInfo < ApplicationRecord
  ...
  scope :host_task_counters, -> { where(info_type: 8) }
